...
const protect = async (req, res, next) => {
  ...
  try {
    const decoded = jwt.verify(token, process.env.JWT_SECRET || 'supportflow_super_secret_jwt_key_phase1');
...
/**
 * Middleware to authorize requests by role.
 * @param  {...string} roles Allowed roles ('customer', 'agent')
 */
